...
const dadosAcesso = ref<IEntrarRequest>({ emailTelefone: '', senha: '' })
const resolver = zodResolver(loginSchema)
const router = useRouter()

const entrar = async (event: any) => {
  if (event.valid) {
    const resp = await autenticacaoStore.entrar(event.values)
    if (resp.status !== 200) {
      return
    }
  }
  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-md align-content-center" style="height: 30rem">
        <h1 class="d-block d-md-none logo-font text-center mb-4">Gestão de Gastos</h1>
        <Form
          v-slot="$form"
          :resolver="resolver"
          :initialValues="dadosAcesso"
          @submit="entrar"
